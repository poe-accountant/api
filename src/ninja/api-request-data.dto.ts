...
export type ApiResultResponseData<T extends ApiEndpoint> =
    T extends ApiEndpoint.PriceItemRequest ? [PriceItemRequest, PriceItemResponse] :
    T extends ApiEndpoint.PriceTabRequest ? [PriceTabRequest, PriceTabResponse] :
    T extends ApiEndpoint.PriceItemResult ? [PriceItemResultRequest, PriceItemResultResponse] :
    T extends ApiEndpoint.PriceTabResult ? [PriceTabResultRequest, PriceTabResultResponse] :
    never;

export type ApiRequest<T extends ApiEndpoint> =
    ApiResultResponseData<T>[0];

export type ApiResponse<T extends ApiEndpoint> =
    ApiResultResponseData<T>[1];
